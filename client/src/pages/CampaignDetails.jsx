    ...
    <div>
      ...
      <div className="mt-[60px] flex lg:flex-row flex-col gap-5">
        <div className="flex-[2] flex flex-col gap-[40px]">
          <div>
            ...
            <div className="mt-[20px] flex flex-row items-center flex-wrap gap-[14px]">
              <div className="w-[52px] h-[52px] flex items-center justify-center rounded-full bg-[#e8b8e8] cursor-pointer">
                <img src={thirdweb} alt="user" className="w-[60%] h-[60%] object-contain"/>
              </div>
              <div>
                <h4 className="font-epilogue font-semibold text-[14px] text-[#565656] break-all">{state.owner}</h4>
          ...
          <div>
            <h4 className="font-epilogue font-semibold text-[18px] text-black uppercase">Story</h4>
              ...
              <div className="mt-[20px]">
                <p className="font-epilogue font-normal text-[16px] text-[#565656] leading-[26px] text-justify">{state.description}</p>
              </div>
          </div>
              ...
              <div className="mt-[20px] flex flex-col gap-4">
                {donators.length > 0 ? donators.map((item, index) => (
                  <div key={`${item.donator}-${index}`} className="flex justify-between items-center gap-4">
                    <p className="font-epilogue font-normal text-[16px] text-[#565656] leading-[26px] break-ll">{index + 1}. {item.donator}</p>
                    <p className="font-epilogue font-normal text-[16px] text-[#808191] leading-[26px] break-ll">{item.donation}</p>
                  </div>
                )) : (
                  <p className="font-epilogue font-normal text-[16px] text-[#565656] leading-[26px] text-justify">No donators yet. Be the first one!</p>
                )}
              </div>
          </div>
        </div>
        ...
        <div className="flex-1">
          ...
          <div className="mt-[20px] flex flex-col p-4 bg-[#f8e6f8] rounded-[10px]">
            ...
            {remainingDays >= 0 && compareAmounts() ? (
              <input 
                ...
                placeholder="ETH 0.1"
                step="0.01"
                className="w-full py-[10px] sm:px-[20px] px-[15px] outline-none border-[1px] border-[#3a3a43] bg-transparent font-epilogue text-black text-[18px] leading-[30px] placeholder:text-[#969ba6] rounded-[10px]"
                value={amount}
                onChange={(e) => setAmount(e.target.value)}
              /> ) : (<h1></h1>)}

              <div className="my-[20px] p-4 bg-[#e8b8e8] rounded-[10px]">
                <h4 className="font-epilogue font-semibold text-[14px] leading-[22px] text-black">Back it because you believe in it.</h4>
                <p className="mt-[20px] font-epilogue font-normal leading-[22px] text-[#565656]">Support the project for no reward, just because it speaks to you.</p>
              </div>
                <CustomButton 
                btnType="button"
