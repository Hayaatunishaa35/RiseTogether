...
import { logo, sun, Rise } from '../assets';
import { navlinks } from '../constants';

const Icon = ({ styles, name, imgUrl, isActive, disabled, handleClick }) => (
  // <div className={`${isActive && isActive === name /*&& 'bg-[#2c2f32]'*/} ${!disabled && 'cursor-pointer'} ${styles}`} onClick={handleClick}>
  <div className={`w-[auto] h-[auto] py-4 rounded-[10px] ${isActive && isActive === name && 'bg-[#e8b8e8]'} ${!disabled && 'cursor-pointer'} ${styles}`} onClick={handleClick}>
...
const Sidebar = () => {
  ...
  return (
    ...
    <div className="min-h-screen flex flex-col flex-auto flex-shrink-0 antialiased bg-[#1c1c24] text-gray-800">
        <div className="fixed flex flex-col top-0 left-0 w-21 bg-[#f8e6f8] h-full border-r">
            <div className="flex items-center justify-center h-14 border-b mt-10">
                <Link to="/">
                    <Icon styles={"w-24 md:w-auto"} imgUrl={Rise} />
                </Link>
            </div>
